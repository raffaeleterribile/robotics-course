...
Reading guide: start with `meta/info.json` to understand the schema; then inspect `meta/stats.json` for normalization; finally, peek at one file in `data/` and `videos/` to connect the dots. You can also do this for all datasets available on the Hugging Face Hub, to get a sense of the peculiarities of a dataset without even having to download it.
